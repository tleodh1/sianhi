...
(function(){
 ...
 HangulRunner.Renderer.prototype.paint=function(e){const t=performance.now();paint.call(this,e);const cost=performance.now()-t;frames++;total+=cost;max=Math.max(max,cost);if(previous)interval+=Math.min(1000,t-previous);previous=t;if(t-lastText>1000){lastText=t;const r=document.querySelector('.hr-shell'),canvas=r?.querySelector('canvas'),touch=r?.querySelector('.hr-touch'),cr=canvas?.getBoundingClientRect(),tr=touch?.getBoundingClientRect();metrics.textContent=`${width.value}px · 그리기 ${(total/frames).toFixed(1)}ms · ${frames}프레임 · ${r?.dataset.playerState||''}`;metrics.dataset.paintMeanMs=(total/frames).toFixed(2);metrics.dataset.paintMaxMs=max.toFixed(2);metrics.dataset.frames=frames;metrics.dataset.canvasWidth=cr?.width.toFixed(1);metrics.dataset.controlsOverlap=String(!!(cr&&tr&&tr.top<cr.bottom));metrics.dataset.contentOverflow=String(r?r.scrollWidth>r.clientWidth+1:false);}};
