...
const context={window:{},state:{stage100:{},records:{}},Math:Object.create(Math)};context.Math.random=Math.random;vm.createContext(context);
for(const file of ['core','content'])vm.runInContext(fs.readFileSync('js/curriculum/'+file+'.js','utf8'),context);
const positions=[];
for(let i=0;i<100;i++){
 const q=context.window.SianCurriculum.question('연산',i%100+1),answer=String(q.ans);
