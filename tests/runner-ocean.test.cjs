...
a.equal(H.worldStages.length,41);a.equal(H.worldStages.filter(s=>s.worldId===2&&s.boss).map(s=>s.id).join(),'2-9');
const before=JSON.stringify({stages:{'2-4':{boss:true,rating:3}},worldRewards:{2:{earnedAt:1}}}),saved=JSON.parse(before);
...
a.equal(biome.size,9);for(const p of ['inflate','pinch','jelly','ink','shark','eel'])a(patterns.has(p));
const ceiling=new H.WorldEngine(H.buildWorldStage('2-8'));ceiling.stage.enemies=[];const roof=ceiling.stage.platforms.find(p=>p.kind==='reef-ceiling');ceiling.player.x=roof.x+20;ceiling.player.y=roof.y+roof.h+5;step(ceiling,100,{jump:true});a(ceiling.player.y>=roof.y+roof.h-1,'solid cave ceiling stops ascent');
const combat=new H.WorldEngine(H.buildWorldStage('2-8'));combat.player.invincible=999;
...
const boss=new H.WorldEngine(H.buildWorldStage('2-9'));H.enterBossArena(boss);boss.player.invincible=999;step(boss,280);a.equal(boss.sceneState,'BOSS_STAGE');
...
console.log('PASS ocean: 9 input-driven full learning routes, >40s travel, terrain, clam, currents, warning/attacks, cave collision, complete boss lifecycle, legacy unlock preservation');
