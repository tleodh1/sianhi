...
({e}=arena('blaster'));e.player.build.parts.back='boost';e.enemy.x=2;e.enemy.z=5;hp=e.enemy.hp;e.attack(e.player,true);assert.equal(e.projectiles.length,0,'skill charges before launch');tick(e,.74);assert.equal(e.projectiles.length,1);assert.equal(e.projectiles[0].kind,'energyBall');assert.ok(e.projectiles[0].position&&e.projectiles[0].velocity&&e.projectiles[0].direction);assert.ok(e.projectiles[0].velocity.x>0,'projectile snapshots actual target direction');assert.ok(e.events.skillCharge&&e.events.launch);tick(e,.7);assert.ok(e.enemy.hp<hp,'projectile collision applies delayed damage');assert.ok(e.events.hit);
...
({e}=arena('blaster'));e.enemy.z=5;e.enemy.guardWanted=true;hp=e.enemy.hp;e.attack(e.player);tick(e,.7);assert.equal(e.enemy.hp,hp,'energy shield blocks projectile damage');assert.ok(e.events.guardHit);assert.ok(e.enemy.shield<e.enemy.shieldMax);assert.ok(e.particles.some(p=>p.kind==='guard'));
({e}=arena('blaster'));e.enemy.z=5;e.enemy.shield=1;e.enemy.guardWanted=true;e.attack(e.player,true);tick(e,1.5);assert.ok(e.events.shieldBreak,'strong skill breaks depleted shield');
({e}=arena('hammer'));e.enemy.z=4;e.attack(e.player,true);tick(e,.7);assert.equal(e.projectiles[0].kind,'shockwave');
({e}=arena('blaster'));e.player.build.parts.back='pack';e.enemy.z=5;e.attack(e.player,true);tick(e,1.05);assert.equal(e.projectiles.filter(p=>p.kind==='missile').length,4,'energy pack launches visible missile barrage');
