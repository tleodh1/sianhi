...
({e}=arena('blaster'));e.enemy.z=5;hp=e.enemy.hp;e.attack(e.player,true);assert.equal(e.projectiles.length,0,'skill charges before launch');tick(e,.4);assert.equal(e.projectiles.length,1);assert.equal(e.projectiles[0].kind,'energyBall');assert.ok(e.events.skillCharge&&e.events.launch);tick(e,.6);assert.ok(e.enemy.hp<hp,'projectile collision applies delayed damage');assert.ok(e.events.hit);
...
({e}=arena('hammer'));e.enemy.z=4;e.attack(e.player,true);tick(e,.36);assert.equal(e.projectiles[0].kind,'shockwave');
