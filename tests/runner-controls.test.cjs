...
for(let cycle=0;cycle<3;cycle++){
 ...
 assert.equal(document.l.get('keydown').size,1);assert.equal(document.l.get('keyup').size,1);
   document.emit('keydown',{code:'ArrowRight'});assert(c.read().right);document.emit('keyup',{code:'ArrowRight'});now+=200;assert(!c.read().right);document.emit('keydown',{code:'KeyF'});assert(c.read().fire);document.emit('keyup',{code:'KeyF'});now+=200;assert(!c.read().fire);
   buttons[1].emit('pointerdown',{pointerId:11,pointerType:'touch'});buttons[2].emit('pointerdown',{pointerId:12,pointerType:'touch'});buttons[4].emit('pointerdown',{pointerId:13,pointerType:'touch'});assert(c.read().right&&c.read().jump&&c.read().fire);
 buttons[1].emit('pointercancel',{pointerId:11});now+=200;assert(!c.read().right&&c.read().jump);
   window.emit('blur');assert(!c.read().jump&&!c.read().fire);assert.equal(paused,1);
