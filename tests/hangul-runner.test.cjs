...
test('letter, coin and power collisions collect only once',()=>{const e=make();for(const kind of ['letter','coin','power']){const i=e.stage.items.find(i=>i.kind===kind);e.player.x=i.x;e.player.y=i.y;e.step(1/120);e.step(1/120);assert(e.collected.has(i.id));}assert.equal(e.letterCount,1);assert.equal(e.coins,1);assert.equal(e.player.powerState,'big');});
