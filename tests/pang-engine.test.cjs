...
for(const [cells,special] of [[[0,1,2,3],'column'],[[0,7,14,21],'row'],[[0,1,2,3,4],'color'],[[1,8,15,14,16],'tnt']]){const e=fixture();cells.forEach(i=>e.board[i].type=7);e.resolve(P.groups(e.board));assert.ok(e.board.some(c=>c?.special===special),special);}
