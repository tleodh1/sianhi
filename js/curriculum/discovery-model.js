(function(global){
 ...
 if(subject==='영어'){
  ...
  if(ch===1||ch===8){q.mode='habitat';q.items=ch===1?['fish','dog','whale','rabbit']:['polar','camel','fish','rabbit'];q.targets=(ch===1?['ocean','forest']:['ice','desert','ocean','forest']).map(x=>({id:x,key:x}));q.items.forEach(x=>q.expected[x]=habitats[x]);q.prompt='동물 친구를 알맞은 집으로 데려다 줘.';q.explanation='동물마다 살기에 알맞은 환경이 달라.';}
