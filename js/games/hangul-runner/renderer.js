(function(HR){
  ...
  HR.Renderer=class {
    ...
    arena(e){const g=this.g,w=this.width,h=this.height,t=e.elapsed,theme=e.stage.theme,b=e.boss,floor=theme==='ocean'?430:420;g.setTransform(this.dpr,0,0,this.dpr,0,0);const palette={forest:['#120b18','#531b1b','#ff6a24'],ocean:['#021b38','#083c5f','#42d7e8'],sky:['#17122f','#403a82','#c6e4ff'],underground:['#070b18','#26254d','#91ffe2']}[theme]||['#120b18','#531b1b','#ff6a24'];const bg=g.createLinearGradient(0,0,0,h);bg.addColorStop(0,palette[0]);bg.addColorStop(.7,palette[1]);bg.addColorStop(1,'#05060b');g.fillStyle=bg;g.fillRect(0,0,w,h);
      ...
      for(const q of b?.choices||[]){const pulse=1+Math.sin(t*5+q.x)*.05;g.save();g.translate(q.x+q.w/2,q.y+q.h/2);g.scale(pulse,pulse);g.shadowColor=q.correct?'#ffe872':'#a176ff';g.shadowBlur=18;g.fillStyle='#fff3bd';g.strokeStyle='#ffbd4d';g.lineWidth=4;g.beginPath();g.arc(0,0,32,0,Math.PI*2);g.fill();g.stroke();g.fillStyle='#3c2947';g.font=`900 ${q.text.length>2?16:27}px Pretendard`;g.textAlign='center';g.textBaseline='middle';g.fillText(q.text,0,1);g.restore();}
      ...
      for(const q of e.projectiles||[]){g.save();g.shadowColor='#fff5a8';g.shadowBlur=25;g.fillStyle='#fff9b5';g.beginPath();g.arc(q.x+15,q.y+11,12,0,Math.PI*2);g.fill();g.strokeStyle='#5ef2ff';g.lineWidth=6;g.beginPath();g.moveTo(q.x-18,q.y+11);g.lineTo(q.x+4,q.y+11);g.stroke();g.restore();}
      ...
      if(b){g.fillStyle='#180e18cc';g.beginPath();g.roundRect(w*.18,18,w*.64,72,18);g.fill();g.fillStyle='#fff';g.textAlign='center';g.font='900 17px Pretendard';g.fillText(b.name,w/2,43);g.fillStyle='#3a2632';g.fillRect(w*.25,54,w*.5,18);g.fillStyle=b.phase===3?'#ff362d':'#ff674f';g.fillRect(w*.25+3,57,(w*.5-6)*(b.hp/b.maxHp),12);if(e.sceneState==='BOSS_STAGE'){g.fillStyle='#fff4c4';g.font='900 20px Pretendard';g.fillText(b.prompt,w/2,118);}if(e.sceneState==='BOSS_INTRO'){g.fillStyle='#fff';g.font='900 32px Pretendard';g.fillText('BOSS BATTLE',w/2,150);}if(e.sceneState==='BOSS_DEFEATED'){g.fillStyle='#fff0a0';g.font='900 30px Pretendard';g.fillText('보스를 물리쳤어!',w/2,155);}if(e.sceneState==='EXIT_OPEN'){g.fillStyle='#fff0a0';g.font='900 24px Pretendard';g.fillText('문이 열렸어! 직접 문으로 이동해요',w/2,145);}if(b.state==='telegraph'){g.fillStyle='#ffb56b';g.font='800 17px Pretendard';g.fillText({charge:'돌진을 피해요!',projectile:'에너지 탄을 피해요!',slam:'위험 표시에서 벗어나요!',shockwave:'충격파를 뛰어넘어요!'}[b.attack],w/2,h-30);}}
    }
