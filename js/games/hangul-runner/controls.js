(function(HR){
  HR.Controls=class {
    constructor(root,onPause,onGesture){
      ...
      this.keyMap={ArrowLeft:'left',ArrowRight:'right',ArrowUp:'jump',Space:'jump',ArrowDown:'down',KeyF:'fire',ShiftLeft:'fire',ShiftRight:'fire'};const key=this.keyMap;
      bind(document,'keydown',e=>{if(e.target.matches('select,input,textarea'))return;if(e.code==='KeyP'){e.preventDefault();if(!e.repeat)onPause();return;}const action=key[e.code];if(!action)return;e.preventDefault();this.pressed.add(e.code);this.pulse[action]=performance.now()+110;onGesture();});
      bind(document,'keyup',e=>{const action=key[e.code];if(action){e.preventDefault();this.pressed.delete(e.code);}});
    ...
    read(){const now=performance.now(),pointer=new Set(this.pointers.values());return Object.fromEntries(['left','right','jump','down','fire'].map(k=>[k,[...this.pressed].some(code=>this.keyMap[code]===k)||pointer.has(k)||(this.pulse[k]||0)>now]));}
