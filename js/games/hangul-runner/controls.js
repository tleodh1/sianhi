(function(HR){
  HR.Controls=class {
    constructor(root,onPause,onGesture){
      ...
      const key={ArrowLeft:'left',ArrowRight:'right',ArrowUp:'jump',Space:'jump',ArrowDown:'down'};
    ...
    read(){const now=performance.now(),pointer=new Set(this.pointers.values());return Object.fromEntries(['left','right','jump','down'].map(k=>[k,this.pressed.has(k)||pointer.has(k)||(this.pulse[k]||0)>now]));}
