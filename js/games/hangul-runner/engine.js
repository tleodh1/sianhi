  ...
  HR.Engine=class {
    ...
    step(dt,input={}) {
      ...
      for(const item of this.stage.items){if(item.dropReward&&!this.collected.has(item.id)){
        ...
        if(item.kind==='coin')this.coins++;if(item.kind==='star')this.stars++;if(item.kind==='flight'){p.flight=true;p.flightTime=12;this.setPose('power-up',.45);}if(item.kind==='power'){this.grow();}this.emit(item.kind,{item});}
