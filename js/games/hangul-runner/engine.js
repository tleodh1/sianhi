...
(function(HR){
  ...
  HR.Engine=class {
    ...
    damage(lethalSmall=false){const p=this.player;if(p.invincible>0||this.status!=='playing')return;
      ...
      else {p.hp=lethalSmall?0:p.hp-1;this.setPose('hurt',.4);this.emit('hurt');}
    ...
    step(dt,input={}) {
      ...
      for(const item of this.stage.items){if(item.contained||this.collected.has(item.id)||!hit(p,item)||(this.canCollect&&!this.canCollect(item)))continue;this.collected.add(item.id);this.burst(item.x+item.w/2,item.y+item.h/2);
        if(item.kind==='coin')this.coins++;if(item.kind==='star')this.stars++;if(item.kind==='flight'){p.flight=true;this.setPose('power-up',.45);}if(item.kind==='power'){p.powerState='big';this.setPose('power-up',.45);}this.emit(item.kind,{item});}
