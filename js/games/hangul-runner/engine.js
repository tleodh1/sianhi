  ...
  HR.Engine=class {
    ...
    step(dt,input={}) {
      ...
      for(const item of this.stage.items){if(item.dropReward&&!this.collected.has(item.id)){
        ...
        for(const floor of this.stage.platforms.filter(a=>a.kind==='ground'))if(item.x+item.w>floor.x&&item.x<floor.x+floor.w&&oldBottom<=floor.y&&item.y+item.h>=floor.y){item.y=floor.y-item.h;item.velocityY=0;item.dropReward=false;break;}
