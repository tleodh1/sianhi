...
(function(HR){
  ...
  HR.Engine=class {
    ...
    fireStar(input={}){const p=this.player;if(p.powerState!=='star_power'||this.fireCooldown>0||this.projectiles.filter(q=>q.kind==='star-power').length>=3||this.status!=='playing')return false;const speed=470,w=28,h=28,start={x:p.x+p.w/2+p.facing*p.w*.42,y:p.y+p.h*.55};let dx=p.facing,dy=0;
      const candidates=[...this.stage.enemies,...(this.boss&&this.boss.state!=='gone'?[this.boss]:[])].filter(e=>!e.defeated&&!e.retracted).map(e=>({e,x:e.x+e.w/2-start.x,y:e.y+e.h/2-start.y})).filter(t=>t.x*p.facing>0&&Math.hypot(t.x,t.y)<=420&&Math.abs(t.y)<=Math.abs(t.x)*Math.tan(Math.PI/4)).sort((a,b)=>Math.hypot(a.x,a.y)-Math.hypot(b.x,b.y));
      if(candidates.length){const t=candidates[0],length=Math.hypot(t.x,t.y);dx=t.x/length;dy=t.y/length;}
      this.projectiles.push({kind:'star-power',team:'player',owner:'player',damage:1,direction:{x:dx,y:dy},speed,lifetime:1.7,x:start.x-w/2,y:start.y-h/2,w,h,radius:14,vx:dx*speed,vy:dy*speed,life:1.7});this.burst(start.x,start.y);this.fireCooldown=.48;this.setPose('power-up',.16);this.emit('starShot',{direction:{x:dx,y:dy}});return true;}
    ...
    tickPlayerProjectiles(dt){this.projectiles=this.projectiles.filter(q=>{const steps=Math.max(1,Math.ceil(Math.hypot(q.vx,q.vy)*dt/Math.max(4,Math.min(q.w,q.h)/3)));for(let step=0;step<steps;step++){q.x+=q.vx*dt/steps;q.y+=q.vy*dt/steps;q.life-=dt/steps;q.lifetime=q.life;if(q.life<=0||q.x<-80||q.x>this.stage.length+80||q.y<-100||q.y>(this.stage.killY||720)+80)return false;
    ...
    step(dt,input={}) {
      ...
      for(const item of this.stage.items){if(item.dropReward&&!this.collected.has(item.id)){
        ...
        if(item.kind==='coin')this.coins++;if(item.kind==='star'){this.stars++;if(this.stage.worldId===4&&!this.jumpBoostUnlocked){this.jumpBoostUnlocked=true;p.jumpForce=790;this.emit('jumpBoost',{item,force:790});}}if(item.kind==='flight'){p.flight=true;p.flightTime=12;this.setPose('power-up',.45);}if(item.kind==='power')this.grow();if(item.kind==='powerStar')this.gainPowerStar();this.emit(item.kind,{item});}
