...
(function(H){
 ...
 H.decorateWorldStage=s=>{decorate(s);if(s.worldId!==2)return;
  s.killY=660;s.oceanDepth=(s.number-1)/8;s.oceanShells=[];
  ...
  s.words.forEach((word,i)=>{const x=i*s.span,letter=s.items.find(t=>t.id===`letter-${i}`);letter.x=x+245;letter.w=word.length>3?152:98;letter.h=word.length>6?112:82;letter.y=[135,240,335][(i+s.number-1)%3];
  ...
  s.enemies.forEach((e,i)=>{const kind=e.originalKind;e.homeY=Math.min(420,e.homeY);if(kind==='reef-crab'){e.homeY=464;e.y=464;e.behavior='pinch';}
