...
(function(H){
 ...
 H.finishStagePacing=function(s){
  ...
  rowFractions.forEach((fraction,i)=>{const x=end*fraction,y=(s.worldId===4&&s.boss)?330:floor-H.PLAYER_FORMS.big.h-38-48;
  ...
  if(s.worldId===4){
   ...
   // 4-4 block rows need a real step-up path so SMALL form can stand on the bricks and collect popped letters.
   if(s.boss){
    for(const b of rewardBlocks){s.platforms.push({x:b.x-74,y:372,w:62,h:16,kind:'pipe',oneWay:true});}
   }
