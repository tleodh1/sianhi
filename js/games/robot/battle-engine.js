(function(R){
 ...
 class Fighter{constructor(build,x,z){this.build=JSON.parse(JSON.stringify(build));this.stats=R.stats(build);Object.assign(this,{x,z,y:0,vy:0,angle:0,hp:this.stats.hp,energy:this.stats.energy,cooldown:0,hurt:0,flash:0,dash:0,dashCooldown:0,guard:false,guardWanted:false,shield:this.stats.defense*2.4,shieldMax:this.stats.defense*2.4,shieldCooldown:0,shieldRipple:0,attackPose:0,attackState:null,walking:false,knockX:0,knockZ:0});}}
 R.BattleEngine=class{
  ...
  tick(dt){if(this.over)return;dt=clamp(dt,0,.05);this.time+=dt;this.shake=Math.max(0,this.shake-dt);this.player.guardWanted=this.input.guard;this.move(this.player,this.input.x,this.input.z,dt);this.ai(dt);
   for(const f of [this.player,this.enemy]){for(const key of ['cooldown','hurt','flash','dash','dashCooldown','attackPose','shieldTime','shieldCooldown','shieldRipple'])f[key]=Math.max(0,(f[key]||0)-dt);if(f.attackPose===0)f.attackState=null;const wantsGuard=(f.guardWanted||f.shieldTime>0)&&f.shieldCooldown===0&&f.energy>0;f.guard=wantsGuard;if(f.guard){f.energy=clamp(f.energy-9*dt,0,f.stats.energy);if(f.energy===0)this.breakShield(f);}else{f.energy=clamp(f.energy+7*dt,0,f.stats.energy);if(f.shieldCooldown===0)f.shield=clamp(f.shield+f.shieldMax*.18*dt,0,f.shieldMax);}if(f.y>0||f.vy>0){f.vy-=15*dt;f.y=Math.max(0,f.y+f.vy*dt);if(f.y===0)f.vy=0;}}
