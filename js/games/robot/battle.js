(function(R){
  R.startBattle=function(root,build,difficulty='normal'){
    ...
    const canvas=root.querySelector('canvas');let view;
    ...
    Session.cleanups.push(()=>{ended=true;disposeControls();view.dispose();});
    function loop(now){if(ended)return;const dt=(now-last)/1000;last=now;if(!document.hidden)engine.tick(dt);view.render();
      ...
      root.querySelector('[data-state]').textContent=engine.player.guard?'방어 중':difficulty.toUpperCase();
