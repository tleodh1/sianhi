...
(function(HR){
  ...
  HR.start=async function(){
    ...
    function event(e){chime(e.type);if(e.type==='letter'){const i=e.item.index,el=$(`[data-letter="${i}"]`);el.textContent=e.item.text;el.classList.add('collected');el.setAttribute('aria-label',e.item.text+' 모음');const trail=$('.hr-letters');if(el.offsetLeft>trail.scrollLeft+trail.clientWidth-65)trail.scrollLeft=el.offsetLeft-trail.clientWidth+80;message(e.item.text+'! 잘 찾았어!');if(sound)say(e.item.text);}
