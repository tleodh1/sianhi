...
(function (C) {
  ...
  C.layout = function () {
    ...
    return ids.map((id,i)=>{const layer=Math.floor(i/9),column=i%9,z=[.89,.64,.38,.12][layer]+((i%3)-1)*.014,x=-208+column*52+(layer%2?22:0),toy={instanceId:`toy-${i}`,id,x,z,height:0,vy:0,tilt:tilts[i],scale:[.94,1.06,1.18,1.34,1.55,1.82][(i*5+layer)%6]+(layer===3?.08:0),pose:i%6===0?'side':i%9===0?'upside-down':i%5===0?'lean':'upright',won:false};return C.constrainToy(toy);});
  };
