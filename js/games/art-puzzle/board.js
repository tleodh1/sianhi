  ...
  A.play = async function (root, art, level, scope, onFinish, onBack) {
    ...
    const complete = () => {
      ...
      scope.later(() => {
        ...
        box.hidden = false;
        box.innerHTML = `<small>나의 미술관에 새겨진 한 장면</small><h2>${art.title}</h2><p>${art.artist} · ${art.country} · ${art.year}</p><strong class="art-result-stars">${"★".repeat(e.rating())}${"☆".repeat(3 - e.rating())}</strong><p>${art.story}</p><p class="art-look">${art.look}</p><p>${Math.floor(e.seconds)}초 · 힌트 ${e.hints}회 · ${reward ? `별 +${reward}` : "최고 기록을 간직했어요"}</p><a class="art-source" href="${art.source}" target="_blank" rel="noopener">${art.museum || "미술관"} 원본 · Public Domain</a><div><button data-next class="art-primary">다른 작품 만나기 →</button></div>`;
